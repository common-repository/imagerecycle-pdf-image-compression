...
class wpImageRecycle
{
    ...
    public function __construct()
    {
        ...
        add_filter('wp_generate_attachment_metadata', array(&$this,'generateMetadata'), 99);
    ...
    function wpio_auto_optimize()
    {
        ...
        $count = $wpio_queue->dbCount();
        $this->write_log('queue count:'.$count);
        while (!empty($count)) {
    ...
    public function doActionUnqueued()
    {
        ...
        if (!current_user_can('edit_posts')) {
            wp_send_json(array('status' => false, 'msg' => esc_html__('Not have permission!', 'wpio')));
        }
    ...
    public function wpio_add_menu_page()
    {
        ...
        add_submenu_page('', 'Folder tree', 'Folder tree', 'manage_options', 'wpir-foldertree', array( $this, 'folderTree' ));
    }
...
<form id="wpio-filter" method="get"  autocomplete="off">
    ...
    <div class="wpio-filter">
        ...
        <option value="" ><?php _e('All', 'wpio'); ?></option>
                <option value="no" <?php if ($filters['optimized']=='no') {
                    echo $selectedAtr;
                                   }?> ><?php _e('Not optimized', 'wpio'); ?></option>
                <option value="yes" <?php if ($filters['optimized']=='yes') {
                    echo $selectedAtr;
                                    }?>><?php _e('Optimized', 'wpio'); ?></option>
            </select>
        ...
        <option value="" ><?php _e('All files', 'wpio'); ?></option>
                ...
                <option value="png" <?php if ($filters['filetype']=='png') {
                    echo $selectedAtr;
                                    }?> ><?php _e('PNG files', 'wpio'); ?></option>
                <?php endif ?>
                <?php if (in_array('jpg', $allowed_ext)) : ?>
                <option value="jpg" <?php if ($filters['filetype']=='jpg') {
                    echo $selectedAtr;
                                    }?> ><?php _e('JPG files', 'wpio'); ?></option>
                <?php endif ?>
                <?php if (in_array('gif', $allowed_ext)) : ?>
                <option value="gif" <?php if ($filters['filetype']=='gif') {
                    echo $selectedAtr;
                                    }?> ><?php _e('GIF files', 'wpio'); ?></option>
                <?php endif ?>
                <?php if (in_array('pdf', $allowed_ext)) : ?>
                <option value="pdf" <?php if ($filters['filetype']=='pdf') {
                    echo $selectedAtr;
                                    }?> ><?php _e('PDF files', 'wpio'); ?></option>
                <?php endif ?>
            </select>
        ...
        <input placeholder="<?php _e('Search', 'wpio'); ?>" id="media-search-input" class="search" name="s" value="<?php echo esc_attr($filters['s']);?>" type="search">
        </div>
    </div>
</form>
            <?php
            ...
            $table->display();
            if ($this->totalNotOptimizedImages==0) {
                $this->totalNotOptimizedImages = 1; //avoid divide zero
            }
    ...
    {
        check_ajax_referer('Image Recycle-options', '_ajax_wpio_nonce');
        ...
        foreach ($images as $image) {
            ...
            if (file_exists(ABSPATH.$image['filename'])) {
                ...
                if (!empty($image['optimized'])) {
                    $sizeBefore = $image['size_before'];
                    if (!$sizeBefore) {
                        $sizeBefore = filesize(ABSPATH.$image['filename']);
                    }
                    $progressVal = round(($sizeBefore - filesize(ABSPATH.$image['filename']))/$sizeBefore*100, 2);
    ...
    //Optimization action
        ...
        } elseif ($this->settings['clean_metadata'] == 'preserve') {
            $fparams['preserve_metas'] = array();
        ...
        $query = $wpdb->prepare(
            'INSERT INTO '.$wpdb->prefix.'wpio_images (`file`,`md5`,api_id,size_before,size_after,`date`,expiration_date) 
